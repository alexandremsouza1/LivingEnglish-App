...
var ms = class MusicService {
  ...
  searchTracks(artists){
    //http://api.musixmatch.com/ws/1.1/track.search?q_artist=justin bieber&page_size=3&page=1&s_track_rating=desc
    let url_req = `${API_ROOT}/${TRACK_SEARCH}?q_track=${artists}&page=1&page_size=3&_track_rating=desc`;
    axios.get(url_req,{
        headers: {
          'apikey': `${API_KEY}`,
  ...
  getTracks(num_artists, country) {
    ...
    let promise = new Promise((resolve, reject) => {
      axios.get(url_req, {json: true}, (err, res, body) => {
        resolve(res);
        reject(err);
      });
    });
